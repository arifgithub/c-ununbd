 ...
 */
 ...
 function make_tree($menu_data){
  ...
  foreach($menu_data as $key=>$val){
      if($val['parent_id']==0){
      $menu[$val['id']]=array('title'=>ucwords($val['title']),'tips'=>$val['tips'],'status'=>$val['status'],'id_content'=>$val['id_content'],'module_link'=>$val['module_link']);
      	unset($menu_data[$key]);
        if($val['child_id']!=0){
        find_child(&$menu_data,&$menu,$val['id']);
        }
      }
  }
...
return $menu;
 ...
 function find_child($menu_data,$menu,$pid){
    foreach($menu_data as $key=>$val){
        ...
        $menu[$pid]['child'][$val['id']]=array('title'=>ucwords($val['title']),'tips'=>$val['tips'],'status'=>$val['status'],'id_content'=>$val['id_content'],'module_link'=>$val['module_link']);
        $menu_data[$key]='';
        //$menu_data=array_filter($menu_data);

          if($val['child_id']!=0){

         find_child(&$menu_data,&$menu[$pid]['child'],$val['id']);
...
function menu_sort($array=array())
{
